...
export class MegaDemoPlayer {
  ...
  async loadDemo(id: string) {
    ...
    const options: DemoOptions = { id, variantId: 0, logicalWidth: this.logicalWidth, logicalHeight: this.logicalHeight, textures: {}, audioTrackId: 'ambient', bpm: 120, beatOffsetSec: 0, params: {} };
  ...
  updateStatus() {
    ...
    if (modeEl) modeEl.textContent = `Mode: ${this.testMode ? 'Test' : 'Normal'}`;
    if (demoEl) demoEl.textContent = `Demo: ${this.currentDemoId}`;
  }
