export function createSolidTexture(gl: WebGLRenderingContext, r: number, g: number, b: number) {
...
export function createFramebuffer(gl: WebGLRenderingContext, width: number, height: number) {
  ...
  const texture = gl.createTexture();
  if (!framebuffer || !texture) throw new Error('Failed to create framebuffer');
  gl.bindTexture(gl.TEXTURE_2D, texture);
